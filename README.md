...
| 容量控制 | 笔记上限48 KiB，达到80%后新交接需整理或说明必要保留理由 |
...
`PreCompact`记录机器状态，不能代替代理提前保存目标和下一步。70%/80%用量提示来自近期兼容日志样本，是评估信号，不是准确的压缩倒计时；日志格式不兼容、缺失或过期时返回unknown。
...
如果安装文件后来被修改，恢复会拒绝覆盖。检查点schema1/2可读；1.4保存使用schema3，旧版脚本不能继续写新数据。**回退Skill文件不等于安全回退任务状态**，不能回退指针来掩盖已执行操作。
